...
namespace Cine_Ma.Repository
{
    public class ChairRepository : IChairRepository
    {
        ...
        {
            var data = await _context.Chairs.ToListAsync();
            return data;
        }
        ...
        {
            var chairs = await _context.Chairs
                .Where(c => c.Row!.ToLower().Contains(row.ToLower()) && c.RoomId == roomId)
                .ToListAsync();
        ...
        {
            var chairs = await _context.Chairs.Where(c => c.RoomId == roomId)
                .OrderBy(c => c.Row)
                .ToListAsync();
